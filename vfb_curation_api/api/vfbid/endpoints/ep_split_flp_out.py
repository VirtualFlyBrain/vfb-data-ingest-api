...
ns = api.namespace('ep_split_flp_out', description='Operations related to EP/Split FLP-OUT multi neuron')


@ns.route('/')
@api.param('apikey', 'Your valid API Key', required=True)
@api.param('orcid', 'Your ORCID', required=True)
class EpSplitFlpOutResource(Resource):

    @api.response(201, 'EP/Split FLP-OUT successfully created.')
    ...
    def post(self):
        ...
        try:
            if valid_user(apikey, orcid):
                nid = create_ep_split(request.json, orcid, True)
    ...
    @api.param('epsplitid', 'EP/Split id')
